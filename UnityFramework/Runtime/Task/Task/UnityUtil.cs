...
public static class UnityUtil
{
    ...
    public static bool DrillMatch(Vector3 p0, Vector3 p1, Vector3 r0, Vector3 r1, float mop, float mor)
    {
        Vector3 op = p1 - p0;
        float opl = op.magnitude;
        if (opl > mop)
        {
            return false;
        }
        if (Vector3.Dot(r0, r1) < 0)
        {
            return false;
        }
        if (Vector3.Cross(r0, r1).magnitude > mor)
        {
            return false;
        }
        return Vector3.Cross(op, r0).magnitude < 0.004f;
    }
    public static bool DrillMatch(Transform t0, Transform t1, float mop, float mor)
    {
        return DrillMatch(t0.TransformPoint(Vector3.zero), t1.TransformPoint(Vector3.zero), t0.forward, t1.forward, mop, mor);
    }
    ...
    public static string TryGetPath(Transform target)
    {
        if (target == null)
        {
            return "";
        }
        return TryGetPath(target.parent) + "/" + target.name;
    }
#if UNITY_EDITOR
    ...
    public static void RemoveAllComponent<T>(GameObject obj, bool recursive) where T : Component
    {
        T[] wait;
        if (recursive)
        {
            wait = obj.GetComponentsInChildren<T>(true);
        }
        else
        {
            wait = obj.GetComponents<T>();
        }
        for (int i = 0; i < wait.Length; ++i)
        {
            Component.Destroy(wait[i]);
        }
    }
}
