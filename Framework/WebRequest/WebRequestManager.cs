...
namespace Framework.WebRequest
{
    ...
    internal sealed partial class WebRequestManager : FrameworkModule, IWebRequestManager
    {
        ...
        {
            return AddWebRequest(webRequestUri, null, tag, DefaultPriority, null);
        }
        ...
        {
            return AddWebRequest(webRequestUri, postData, tag, DefaultPriority, null);
        }
        ...
        {
            return AddWebRequest(webRequestUri, null, tag, DefaultPriority, userData);
        }
        ...
        {
            return AddWebRequest(webRequestUri, postData, tag, DefaultPriority, userData);
        }
        ...
        public int AddWebRequest(string webRequestUri, byte[] postData, string tag, int priority, object userData)
        {
            ...
            {
                throw new FrameworkException("Web request uri is invalid.");
            }

            if (TotalAgentCount <= 0)
            {
                throw new FrameworkException("You must add web request agent first.");
            }
        ...
        private void OnWebRequestAgentSuccess(WebRequestAgent sender, byte[] webResponseBytes, string webResponseText)
        {
            if (m_WebRequestSuccessEventHandler != null)
            {
                WebRequestSuccessEventArgs webRequestSuccessEventArgs = WebRequestSuccessEventArgs.Create(sender.Task.SerialId, sender.Task.WebRequestUri, webResponseBytes, webResponseText, sender.Task.UserData);
