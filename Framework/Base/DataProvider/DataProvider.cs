...
namespace Framework
{
    ...
    internal sealed class DataProvider<T> : IDataProvider<T>
    {
        ...
        public bool ParseData(string dataString, object userData)
        {
            if (m_DataProviderHelper == null)
            {
                throw new FrameworkException("You must set data helper first.");
            }

            if (dataString == null)
            {
                throw new FrameworkException("Data string is invalid.");
            }
            ...
            catch (Exception exception)
            {
                ...
                throw new FrameworkException(Utility.Text.Format("Can not parse data string with exception '{0}'.", exception.ToString()), exception);
            }
        }
        ...
        public bool ParseData(byte[] dataBytes)
        {
            if (dataBytes == null)
            {
                throw new FrameworkException("Data bytes is invalid.");
            }
        ...
        public bool ParseData(byte[] dataBytes, int startIndex, int length, object userData)
        {
            ...
            catch (Exception exception)
            {
                ...
                throw new FrameworkException(Utility.Text.Format("Can not parse data bytes with exception '{0}'.", exception.ToString()), exception);
            }
        }
